(() => {
      ...
      (function(global, factory) {
        ...
        jQuery.event = {
          ...
          dispatch: function(nativeEvent) {
            var i, j, ret, matched, handleObj, handlerQueue, args = new Array(arguments.length), event2 = jQuery.event.fix(nativeEvent), handlers = (dataPriv.get(this, "events") || Object.create(null))[event2.type] || [], special = jQuery.event.special[event2.type] || {};
            args[0] = event2;
    ...
    if (callback) {
      ...
      if (isShopifySectionReload) {
        document.addEventListener("shopify:section:load", (event2) => {
          if (event2.detail.sectionId === sectionId2) {
            callback();
          }
        });
      }
  ...
  function init() {
    ...
    async function handleClickButton(type, target, event2) {
      event2.preventDefault();
    ...
    addBtns.forEach(function(addBtn) {
      addBtn.addEventListener("click", (event2) => handleClickButton("add", event2.target, event2));
    });
    removeBtns.forEach(function(removeBtn) {
      removeBtn.addEventListener("click", (event2) => handleClickButton("remove", event2.target, event2));
    });
    function inputEvents() {
      ...
      inputs.forEach(function(input) {
        ...
        input.addEventListener("input", debounce((event2) => handleClickButton("change", event2.target, event2), 2e3));
      });
    }
    inputEvents();
    checkMaxCartPage();
